@extends('backend.app')
@section('title', __('گۆڕانکاری لە تێچوو'))
@section('content')
    <div class="page-content">
        <div class="container-xxxl flex-grow-1 container-p-y">
            <h4 class="fw-bold mb-4"><span class="text-muted fw-light">گۆڕانکاری لە تێچوو</span></h4>
            <div class="row">
                <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-body">
                            <x-auth-validation-errors class="mb-4" :errors="$errors" />
                            <form method="POST" action="{{ route('techus.update', $techu->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                        ...
                                        <select class="form-select" id="name" name="name">
                                            @foreach ($clients as $client)
                                                <option value="{{ $client->name }}"
                                                    @if ($techu->name == $client->name) selected @endif>{{ $client->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-3 col-md-8">
                                        ...
                                        <select class="form-select" id="type" name="type">
                                            @foreach ($techuTypes as $techus)
                                                <option value="{{ $techus->name }}"
                                                    @if ($techu->type == $techus->name) selected @endif>{{ $techus->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-3 col-md-12">
                                        ...
                                        <div class="input-group date me-2 mb-2 mb-md-0" id="datePickerExample1">
                                            ...
                                            <input type="text" class="form-control border-primary bg-transparent"
                                                name="date" placeholder="ڕێکەوت" value="{{ $techu->date }}">
                                        </div>
                                    </div>
                                    <div class="mb-3 col-md-12">
                                        <label for="amount" class="form-label">تێبینی</label>
                                        <textarea name="comment" class="form-control" cols="30" rows="10"> {{ $techu->comment }} </textarea>
                                    </div>
                                </div>
                                <div class="mt-2">
